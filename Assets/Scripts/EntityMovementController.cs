...
public class EntityMovementController : MonoBehaviour, ICharacterController
{
    ...
    private Vector3? _moveToTarget = null;
    private float _moveToStoppingDistance = 0.1f;
    ...
    public void BeforeCharacterUpdate(float deltaTime)
    {
        ...
        if (_tmpTransientRot != Quaternion.identity)
        {
            transform.rotation = _tmpTransientRot;
        }
    }
                ...
                if (Motor.GroundingStatus.IsStableOnGround)
                {
                    ...
                    if (_moveToTarget.HasValue)
                    {
                        ...
                        else
                        {
                            ...
                            Vector3 desiredVelocity = direction * _moveToMaxSpeed;
                            targetMovementVelocity = Vector3.MoveTowards(currentVelocity, desiredVelocity, _moveToAccel * deltaTime);
    ...
    public void MoveTo(Vector3 targetPosition, float stoppingDistance = 0.1f, float maxSpeed = 10f, float acceleration = 20f, Action onReachedTarget = null)
    {
        ...
        _moveToStoppingDistance = stoppingDistance;
        _moveToMaxSpeed = maxSpeed;
        _moveToAccel = acceleration;
    ...
    {
        // Snap rotation to 8 directions for a more arcade feel
        // if (_moveInputVector.sqrMagnitude > 0f)
        // {
        _tmpTransientRot = transform.rotation;
        Vector3 angles = Motor.TransientRotation.eulerAngles;
        Vector3 newAngles = new Vector3(0f, Mathf.Round(angles.y / 45f) * 45f, 0f);
